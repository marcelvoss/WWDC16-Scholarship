...
- (void)doResize {
    ...
    CGSize newSize = self.fitWithin;
    CGSize outputImageSize = self.imageToResize.size;
    CGFloat widthRatio = newSize.width/outputImageSize.width;
    CGFloat heightRatio = newSize.height/outputImageSize.height;
    
    if (widthRatio > heightRatio)
    {
        newSize=CGSizeMake(outputImageSize.width*heightRatio,outputImageSize.height*heightRatio);
    } else {
        newSize=CGSizeMake(outputImageSize.width*widthRatio,outputImageSize.height*widthRatio);
    }
    
    
    UIGraphicsBeginImageContextWithOptions(newSize, NO, 0.0);
    [self.imageToResize drawInRect:CGRectMake(0,0,newSize.width,newSize.height)];
